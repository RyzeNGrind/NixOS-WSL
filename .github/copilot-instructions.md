...
- **Key files:** `flake.nix`, `configuration.nix`, `home.nix`, WSL-specific modules
...
- **Critical:** Must support SSH from both LAN (`nixos-lan` key) and Tailscale (`tailce65.ts.net`)
...
- `impermanence` — ephemeral root, opt-in persistence
